...
reconstructed_event reconstruct(VecOps::RVec<Jet> &fatjets, //Input  FatJet of FastJet <Jet> type
	...
	VecOps::RVec<Jet> mtchd_trkjets;
	for( auto calojet : fatjets){ 
	get_assoc_trkjets(calojet,smalljets,mtchd_trkjets)
	}
	
	
	std::vector<OxJet> lj_vec=
	view::zip_with(make_jet,fatjets) | view::filter([](const auto &calojet){ //Apply Wrapper for OxJet
	return calojet.p4.Pt() >=200.*GeV and std::abs(calojet.p4.Eta())<2.5; //Apply Boosted Filter
	});	
