...
jet_indices_comb(int n_jets */
/**< [in] Number of jets to choose from */                 //,
    ...
    result.ForeachSlot(
          [&out_trees, &out_vars, &ntag_var, &njets_var, &rwgt_vars,
           ...
           &mc_sf_var](unsigned slot, const reconstructed_event& event) {
              ...
              rwgt_jets |= (action::sort(ranges::ordered_less{},
                                         [](const auto& jet) { return jet.p4.Pt(); })
                            | action::reverse);
              rwgt->pT_2 = rwgt_jets[1].p4.Pt();
              rwgt->pT_4 = rwgt_jets[3].p4.Pt();
              rwgt->eta_i =
                    ranges::accumulate(rwgt_jets, 0., ranges::plus{},
                                       [](const auto& jet) { return std::abs(jet.p4.Eta()); })
                    / 4;
              ...
              std::vector<std::tuple<int, int>> rwgt_jet_pairs =
                    ...
                    | view::remove_if([](auto&& is) { return std::get<0>(is) <= std::get<1>(is); });
              rwgt_jet_pairs |= (action::sort(ranges::ordered_less{}, [&rwgt_jets](auto&& is) {
                  auto i = std::get<0>(is);
                  auto j = std::get<1>(is);
