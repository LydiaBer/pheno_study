...
OUT_DIR = os.getcwd().split("/resolved")[0]+"/outputs/signal_240119"
USE_BATCH = False
...
with open(FILE_LIST, 'r') as filehandle:  
  ...
  for file_path in filecontents:
      ...
      # make short output filename of form resolved_sample.root
      if "signal/nominal" in file_path:
        output_filename = 'resolved_'+file_path.split('nominal/')[1]  
        ...
        output_filename = "resolved_hh_"+"TopYuk_"+TopYuk+"_"+"SlfCoup_"+SlfCoup+"trackJetBTag.root"  
      ...
      #    filter = "xpt200"
      #  output_filename = "resolved_{0}_{1}_{2}".format(filter, file_path.split("/")[-2],file)
