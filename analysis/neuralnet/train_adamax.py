...
def main(argv):
    ...
    param_string = "valsplittest_w_OPadamaxEP"+str(numEpochs)+"BS"+str(batchSize)+"DO"+str(dropoutFraction).replace(".","")
    ...
    for analysis in ["resolved", "intermediate", "boosted"]:
        ...
        # Train the NN
        history = model.fit(xTrainData, yTrainCategories, validation_split=0.2, epochs=numEpochs, batch_size=batchSize, sample_weight=evtWeightsTrain,verbose=2)
