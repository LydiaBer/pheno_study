...
### NOTE this program uses one of the unused split samples as validation dataset
...
def main(argv):
    ...
    param_string = "w_OPsgdEP"+str(numEpochs)+"BS"+str(batchSize)+"DO"+str(dropoutFraction).replace(".","")
    ...
    for analysis in ["resolved", "intermediate", "boosted"]:
        ...
        inFile_val = np.load(trainingDataPath + "/validationData_" + analysis + ".npz")

        signalData = pandas.DataFrame(inFile["sig"])
        signalData["target"] = 0
        signalData_val = pandas.DataFrame(inFile_val["sig"])
        signalData_val["target"] = 0
        print(len(signalData), "signal events found")
        print(len(signalData_val), "signal events for validation")

        backgroundData = pandas.DataFrame(inFile["bkg_4b"])
        backgroundData["target"] = 1
        backgroundData_val = pandas.DataFrame(inFile_val["bkg_4b"])
        backgroundData_val["target"] = 1
        print(len(backgroundData), "4b background events found")
        print(len(backgroundData_val), "4b background events for validation")

        background_2b2j = pandas.DataFrame(inFile["bkg_2b2j"])
        background_2b2j["target"] = 1
        background_2b2j_val = pandas.DataFrame(inFile_val["bkg_2b2j"])
        background_2b2j_val["target"] = 1
        print(len(background_2b2j), "2b2j background events found")
        print(len(background_2b2j_val), "2b2j background events for validation")
        backgroundData = backgroundData.append(background_2b2j)
        backgroundData_val = backgroundData_val.append(background_2b2j_val)

        background_ttbar = pandas.DataFrame(inFile["bkg_ttbar"])
        background_ttbar["target"] = 2
        background_ttbar_val = pandas.DataFrame(inFile_val["bkg_ttbar"])
        background_ttbar_val["target"] = 2
        print(len(background_ttbar), "ttbar background events found")
        backgroundData = backgroundData.append(background_ttbar)
        backgroundData_val = backgroundData_val.append(background_ttbar_val)

        allData = signalData.append(backgroundData, ignore_index = True)
        data_val = signalData_val.append(backgroundData_val, ignore_index = True)

        # Set all event weights to 1 by default
        allData.loc[:, 'mc_sf'] = 1.
        data_val.loc[:, 'mc_sf'] = 1.
        # Apply manual event weighting: numbers that give "reasonable behavior"
        if analysis is "resolved":
            allData.loc[allData.target == 0, 'mc_sf'] = 2.6  # signal
            allData.loc[allData.target == 1, 'mc_sf'] = 0.4  # QCD
            allData.loc[allData.target == 2, 'mc_sf'] = 0.3 # ttbar
        if analysis is "intermediate":
            allData.loc[allData.target == 0, 'mc_sf'] = 4.  # signal
            allData.loc[allData.target == 1, 'mc_sf'] = 0.15  # QCD
            allData.loc[allData.target == 2, 'mc_sf'] = 2. # ttbar
        if analysis is "boosted":
            allData.loc[allData.target == 0, 'mc_sf'] = 4.  # signal
            allData.loc[allData.target == 1, 'mc_sf'] = 0.04  # QCD
            allData.loc[allData.target == 2, 'mc_sf'] = 2.8 # ttbar
        ...
        evtWeightsVal = xValData.pop("mc_sf")
        procTypeVal = xValData.pop("target")
        ...
        xValData = classStd1.fit_transform(xValData)
        ...
        # Train the NN
        history = model.fit(xTrainData, yTrainCategories, validation_data=(xValData, yValCategories), epochs=numEpochs, batch_size=batchSize, sample_weight=evtWeightsTrain,verbose=2)
