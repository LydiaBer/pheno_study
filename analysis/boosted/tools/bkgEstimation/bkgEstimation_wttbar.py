...
def main(trackJetType = "FR"):
  ...
  for sample in sampleList:
    ...
    for region in massregionList:
          for variable in variableList:
            histoName =  region + '_' + variable
            histograms[sample][region][variable] = files[sample].Get(histoName)
            print histograms[sample][region][variable].GetName(),' retrieved from  ', histoName 
  
  # data
  print 'adding bkgds'
  print histograms[s_4bName]['SB']['hcand1_m'].GetNbinsX() 
  print histograms[s_4jName]['SB']['hcand1_m'].GetNbinsX() 
  print histograms[s_2b2jName]['SB']['hcand1_m'].GetNbinsX() 
  print histograms[s_4bName]['SB']['hcand1_m'].GetName() 
  print histograms[s_4jName]['SB']['hcand1_m'].GetName() 
  print histograms[s_2b2jName]['SB']['hcand1_m'].GetName() 
  data_4tag_SB = histograms[s_4bName]['SB']['hcand1_m'] + histograms[s_4jName]['SB']['hcand1_m'] + histograms[s_2b2jName]['SB']['hcand1_m'] 
  #data_bkd_model_for_SB = histograms[dataName]['0tag_SB']['hcand1_m']
  print 'adding lowtag bkgds'
  print histograms[s_4bName]['lowTag_SB']['hcand1_m'].GetNbinsX() 
  print histograms[s_4jName]['lowTag_SB']['hcand1_m'].GetNbinsX() 
  print histograms[s_2b2jName]['lowTag_SB']['hcand1_m'].GetNbinsX() 
  data_bkd_model_for_4b_SB = histograms[s_4bName]['lowTag_SB']['hcand1_m'] + histograms[s_4jName]['lowTag_SB']['hcand1_m'] + histograms[s_2b2jName]['lowTag_SB']['hcand1_m']
  # ttbar
  ttbar_4tag_SB = histograms[ttbarName]['SB']['hcand1_m']
  #ttbar_bkd_model_for_SB = histograms['ttbar']['0tag_SB']['hcand1_m']
  ttbar_bkd_model_for_4b_SB = histograms[ttbarName]['lowTag_SB']['hcand1_m']
  
  
  fitter_4tag = BkgdUtils.BackgroundFitter(trackJetType+"_4tag")
  fitter_4tag.Initialize(data_4tag_SB, ttbar_4tag_SB, data_bkd_model_for_4b_SB, ttbar_bkd_model_for_4b_SB)
  results_4tag = fitter_4tag.RunFit("NTag",outDir)
  
  mu_4tag = results_4tag["muqcd"]
  
  alpha_tt_4tag = results_4tag["topscale"]

  for massregion in ['SB','CR','SR']:
      for variable in variableList:
        #
        # GetHisto
        #
        hist_lowerTag_4tag = histograms[s_4bName]['lowTag_' + massregion][variable] + histograms[s_4bName][ 'lowTag_' + massregion][variable] +histograms[s_2b2jName]['lowTag_' + massregion][variable] 
        ...
        data_4tag = histograms[s_4bName][massregion][variable] + histograms[s_4jName][massregion][variable] +histograms[s_2b2jName][massregion][variable] 

        clone_bkg_4b   = histograms[s_4bName]['lowTag_' + massregion][variable].Clone('scaled_4b_lowtag')
        clone_bkg_4j   = histograms[s_4jName]['lowTag_' + massregion][variable].Clone('scaled_4j_lowtag')
        clone_bkg_2b2j   = histograms[s_2b2jName]['lowTag_' + massregion][variable].Clone('scaled_2b2j_lowtag')

        clone_bkg_4b.Scale(mu_4tag)  
        clone_bkg_4j.Scale(mu_4tag)  
        clone_bkg_2b2j.Scale(mu_4tag)
        #
        # Use 3tag ttbar as template but normalized to alpha_tt_4tag*
        #
        ttbar_4tag = histograms[ttbarName][massregion][variable].Clone("ttbar_4tag_from3tag")

        data_4tag.Add(ttbar_4tag,1.5)

        ttbar_4tag.Scale(alpha_tt_4tag)
        ...
        # Plot 4tag
        #
        MakeDataVsPredictionPlots(data_4tag, ttbar_4tag, clone_bkg_4b, clone_bkg_4j, clone_bkg_2b2j, "t#bar{t}", "bkg 4b", "bkg 4j","bkg 2b2j","", "Events", False, doLogY,trackJetType+"_4tag_"+massregion,"_estimationVSbkg_"+massregion+"_"+variable)
#  9 def MakeDataVsPredictionPlots(data,     l    ttbar,           h1,           h2,             h3,   legttbar,    legh1,    legh2,      legh3, xaxisname, yaxisname, doBlind, doLogY, info, pdftitle):
  ...
  for region in ["SB","CR","SR"]:
    for variable in variableList:
      ...
      histo_ttbar = histograms[ttbarName][region][variable].Clone("ttbar_"+region+"_"+variable)
      outfile.WriteTObject(histo_ttbar)
...
def MakeDataVsPredictionPlots(data, ttbar, h1, h2, h3, legttbar, legh1, legh2, legh3, xaxisname, yaxisname, doBlind, doLogY, info, pdftitle):
  ...
  ttbarC.SetLineColor(1)
  ttbarC.SetFillColor(ROOT.kAzure-9)

  stack.SetTitle('')
  
  stack.Add(ttbarC)
  leg.AddEntry(ttbarC, legttbar, "f")
  ...
  if doBlind:
    ...
    dataC = data.Clone()
    SetDataStyle(dataC)
    leg.AddEntry(dataC, "NTag+"+legttbar, "pe")
    dataC.Draw('P same')
