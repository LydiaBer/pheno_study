...
class TaggedJet{
  ...
  std::vector<OxJet> constituents;        
        ...
        TaggedJet(double M, double pT, double eta, long double phi, bool tag, int numberB)
              : p4(), tagged(tag), ntag(numberB) {
        p4.SetPtEtaPhiM(pT, eta, phi, M);
        }
	TaggedJet(OxJet calojet, std::vector<OxJet> child, int numberB )
              : p4(), tagged(true), ntag(numberB) {
        p4.SetPtEtaPhiM(calojet.p4.Pt(), calojet.p4.Eta(), calojet.p4.Phi(), calojet.p4.M());
...
inline OxJet undo_pseudo(fastjet::PseudoJet &jet){
   ...
   double phi = jet.phi();
   bool tagged;
   if (jet.user_info<MyInfo>().tagged() == 1 ) tagged = true;
   if (jet.user_info<MyInfo>().tagged() == 0 ) tagged = false;
...
inline TaggedJet make_tagged(OxJet &calojet, std::vector<OxJet>& subjets, int ntag){
...
inline void tagging_algo(OxJet& calojet, std::vector<OxJet>& matched_trkjets,TaggedJet tagged_fatjet){
...
inline void get_assoc_trkjets( Jet& calojet, ROOT::VecOps::RVec<Jet> trkjets,
    ...
    for (unsigned int trackJetItr = 0; trackJetItr < pseudo_trkjets.size(); ++trackJetItr) {
    ...
    std::vector<fastjet::PseudoJet> sorted_jets = fastjet::sorted_by_pt(clust_seq.inclusive_jets());
    ...
    for (const auto& constit : newJet_constituents) {
        ...
        if (constit.user_index() >= 0) {
            ...
            std::cout<<"Check user index"<<iter<<std::endl;
	    matched_pseudo_trkjets.push_back(newJet_constituents.at(iter));
	}
    }
    std::cout<<"After Pushing Back newJetconstistuents"<<std::endl;

    // Sort matched jets by pt
    matched_pseudo_trkjets = sorted_by_pt(matched_pseudo_trkjets);
    
    //Undo PseudoJet
    std::vector<OxJet> out_matched = view::zip_with(undo_pseudo, matched_pseudo_trkjets);    
