...
reconstructed_event reconstruct(
  ...
  */
  ...
  for (auto calojet : fatjets) {
    tagging_algo(calojet)
  }

  std::vector<OxJet> lj_vec =
      view::zip_with(make_jet, fatjets) |
      view::filter([](const auto &calojet) { // Apply Wrapper for OxJet
        return calojet.p4.Pt() >= 200. * GeV and
               std::abs(calojet.p4.Eta()) < 2.5; // Apply Boosted Filter
      });
...
int main(int arc, char *argv[]) {
  ...
  auto reconstructed = two_b_jets.Define(
      "event", reconstruct, {"FatJet", "Jet", "Event"}); // Reconstruct Events
  ...
  write_tree(control_result, "control", output_file); // Writing the Control
                                                      // Tree
  ...
  boosted_cutflow.add(u8"Signal", control_result.Count());
  boosted_cutflow.add(u8"Signal", sideband_result.Count());
