...
inline void flavour_algo(const std::vector<int> b_tag,
                         const std::vector<int> c_tag,
                         const std::vector<int> ph_tag,
                         std::vector<int> s_vec_tag) {

  if ((b_tag.at(0) > 0) || (b_tag.at(1) > 0))
    s_vec_tag.at(0) = 1; // BTagged
  if (((c_tag.at(0) > 0) || (c_tag.at(1) > 0)) && (s_vec_tag.at(0) != 1))
    s_vec_tag.at(1) = 1; // CTagged
  if ((((ph_tag.at(0) > 0) || (ph_tag.at(1) > 0)) && (s_vec_tag.at(0) != 1)) &&
      (s_vec_tag.at(1) != 1))
    s_vec_tag.at(2) = 1; // Light Tagged

  return;
}

inline void tagging_algo(Jet &calojet) {

  std::vector<Jet> matched_trkjets;
  for (int i = 0; i < calojet.Constituents.GetEntries(); i++) {
    matched_trkjets.push_back(calojet.Constituents.At(i));
  }

  ranges::sort(matched_trkjets, ranges::ordered_less{},
               [](auto &&jet) { return jet.PT; });
  ranges::reverse(matched_trkjets);

  Jet leading_trkjet = matched_trkjets[0];
  Jet subleading_trkjet = matched_trkjets[1];

  std::vector<Jet> lead_trks = *leading_trkjet.Constituents;
  std::vector<Jet> sublead_trks = *subleading_trkjet.Constituents;

  for (int i = 0; i < leading_trkjet.Constituents.GetEntries(); i++) {
    lead_trks.push_back(*leading_trkjet.Constituents.At(i));
  }

  for (int i = 0; i < subleading_trkjet.Constituents.GetEntries(); i++) {
    sublead_trks.push_back(*subleading_trkjet.Constituents.At(i));
  }
  ...
  std::vector<int> b_tag = {0, 0};
  std::vector<int> c_tag = {0, 0};
  std::vector<int> light_tag = {0, 0};

  for (const auto &trk : lead_trks) {
    // Following PDGId number convention

    if (MCUtils::PID::hasBottom(trk.Flavor))
      b_tag[0]++;
    if (MCUtils::PID::hasCharm(trk.Flavor))
      c_tag[0]++;
    if (MCUtils::PID::isLightParton(trk.Flavor))
      light_tag[0]++;
  }
  ...
  if (switch_tag.at(0) != 0)
    calojet.Flavor = 5;
  if (switch_tag.at(1) != 0)
    calojet.Flavor = 4;
  if (switch_tag.at(2) != 0)
    calojet.Flavor = 22;

  return;
}
/*
inline void get_assoc_trkjets( fastjet::PseudoJet& calojet,
std::vector<fastjet::PseudoJet> trkjets,
                       std::vector<fastjet::PseudoJet>& matched_trkjets, bool
debug = false) {
...
<< std::endl;
    ...
    for (auto noghost : constituents) {
        ...
        noghost.reset_PtYPhiM(noghost.pt(), noghost.rapidity(), noghost.phi(),
0.0);
        if (noghost.E() < 0.) continue;

        // set user index for calo clusters to -1 to differentiate them from
"track" constituents
        // later
    ...
    if (debug)
        std::cout << "calo only input particles size = " <<
input_particles.size() << std::endl;

    // make ghost PseudoJets out of track jet direction
    for (unsigned int trackJetItr = 0; trackJetItr < trkjets.size();
++trackJetItr) {

        fastjet::PseudoJet myghost = trkjets.at(trackJetItr);
        // if( myghost.pt() <= 20.0 || fabs( myghost.rapidity() ) >= 2.5 )
continue;
...
input_particles.size()
                  << std::endl;

    // do ghost association and get list of pseudojet track jets that are
associated
    double                       Rparam        = 1.0;
    fastjet::Strategy            strategy      = fastjet::Best;     // according
to atlas reco
    fastjet::RecombinationScheme recomb_scheme = fastjet::E_scheme; // according
to atlas reco
...
fastjet::sorted_by_pt(clust_seq.inclusive_jets());
    ...
    if (debug) std::cout << "number of sorted jets = " << sorted_jets.size() <<
std::endl;

    fastjet::PseudoJet newJet =
          sorted_jets.at(0); // there are more jets in the vector, but they all
have pT ~0
    if (debug)
        std::cout << "new jet constituent size = " <<
newJet.constituents().size() << std::endl;
    ...
    for (const auto& constit : newJet_constituents) {
        ...
        if (constit.user_index() >= 0) {
            ...
            //    if(trkjets.at(iter).pt() > 20. && fabs(trkjets.at(iter).eta())
< 2.5 )
