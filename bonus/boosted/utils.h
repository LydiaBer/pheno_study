...
inline void flavour_algo(const std::vector<int> b_tag, const std::vector<int> c_tag,
			const std::vector<int> ph_tag,std::vector<int> s_vec_tag){
...
inline void tagging_algo(Jet& calojet){
		...
		//Vector for switching different taggin for the calo jet
                ...
                for(const auto& trk : lead_trks) {
                        ...
                        if (MCUtils::PID::hasBottom(trk.Flavor)) b_tag[0]++;
                        if (MCUtils::PID::hasCharm(trk.Flavor)) c_tag[0]++;
                        if (MCUtils::PID::isLightParton(trk.Flavor)) light_tag[0]++;
                }
                ...
                for(const auto& trk : sublead_trks) {
                        ...
                        if (MCUtils::PID::hasBottom(trk.Flavor))  b_tag[1]++;
                        if (MCUtils::PID::hasCharm(trk.Flavor)) c_tag[1]++;
                        if (MCUtils::PID::isLightParton(trk.Flavor)) light_tag[1]++;
                }


                flavour_algo(b_tag,c_tag,light_tag,switch_tag);

                if (switch_tag.at(0) !=0) calojet.Flavor = 5 ;
                if (switch_tag.at(1) !=0) calojet.Flavor = 4;
                if (switch_tag.at(2) !=0) calojet.Flavor = 22;

                return;

}
/*
inline void get_assoc_trkjets( fastjet::PseudoJet& calojet, std::vector<fastjet::PseudoJet> trkjets,
