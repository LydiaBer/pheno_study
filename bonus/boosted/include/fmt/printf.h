...
class ArgConverter : public ArgVisitor<ArgConverter<T>, void> {
  ...
  public:
    ...
    template <typename U> void visit_any_int(U value) {
        ...
        using internal::Arg;
        typedef typename internal::Conditional<is_same<T, void>::value, U,
                                               T>::type TargetType;
        if (sizeof(TargetType) <= sizeof(int)) {
            ...
            if (is_signed) {
                ...
                arg_.type = Arg::UINT;
                typedef typename internal::MakeUnsigned<TargetType>::Type
                      Unsigned;
                arg_.uint_value =
    ...
    while (*s) {
        ...
        if (*s == '.') {
            ...
            if ('0' <= *s && *s <= '9') {
                ...
                ++s;
                spec.precision_ =
                      internal::PrecisionHandler().visit(get_arg(s));
            }
            else {
                spec.precision_ = 0;
            }
