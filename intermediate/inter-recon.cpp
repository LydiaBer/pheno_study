...
bool one_b_large_two_b_small_cuts(VecOps::RVec<Jet> &jets,
  ...
  if (b_count_fat != 0)
    std::cout << "Fat_b_Jet" << b_count_fat << std::endl;
...
int main(int arc, char *argv[]) {
  ...
  // RDataFrame
  // frame("Delphes","/data/atlas/atlasdata/micheli/4b/Events/run_02/tag_1_delphes_events.root");
  ...
  auto three_jets =
      frame.Filter(one_b_large_two_b_small_cuts, {"Jet", "FatJet"},
                   u8"Intermediate analysis cuts");

  // std::cout<<"flag 2"<<std::endl;
  auto reconstructed =
      three_jets.Define("event", reconstruct, {"Jet", "FatJet", "Event"});
